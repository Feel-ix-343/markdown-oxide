...
impl Settings {
    pub fn new(root_dir: &Path) -> anyhow::Result<Settings> {
        ...
        let settings = Config::builder()
            .add_source(
                File::with_name(&format!("{}/.moxide", root_dir.to_str().ok_or(anyhow!("Can't convert root_dir to str"))?))
                    .required(false)
            )
            .add_source(
                File::with_name("~/.config/moxide/settings")
                    .required(false)
            )
            .set_default("dailynote", obsidian_daily_note.unwrap_or("%Y-%m-%d".to_string())).map_err(|err| anyhow!("Failed to parse due to: {err}"))?
            .build().map_err(|err| anyhow!("Build err: {err}"))?;
...
fn obsidian_dailynote_converted(root_dir: &Path) -> Option<String> {
    ...
    let config: Option<HashMap<String, String>> = file.and_then(|file| {
