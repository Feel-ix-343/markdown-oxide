...
pub fn goto_definition(vault: &Vault, cursor_position: Position, path: &Path) -> Option<Vec<Location>> {
    ...
    let referenced_referenceables = positions.iter().filter(|i| reference.references(&vault.root_dir(), path, i));

    return Some(referenced_referenceables.filter_map(|linkable| Some(Location{uri: Url::from_file_path(linkable.get_path().to_str()?).unwrap(), range: linkable.get_range()})).collect());
}
