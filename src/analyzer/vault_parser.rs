...
pub fn parse_vault(dir: &str) -> Result<(), std::io::Error>  {
    ...
    let md_files: HashMap<String, MDFile> = dir_path
        ...
        .map(|f| {
            ...
            let source = read(f.path()).unwrap();

            let headings = query_and_links(&source, "(atx_heading heading_content: (inline) @heading);")
                .into_iter()
                .map(|(text_match, links)| MDHeading::new(text_match, links))
                .collect_vec();

            let paragraphs = query_and_links(&source, "(paragraph (_) @paragraph);")
                .into_iter()
