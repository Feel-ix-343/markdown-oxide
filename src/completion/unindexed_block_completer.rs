...
use super::{link_completer::{LinkCompleter, MarkdownLinkCompleter, WikiLinkCompleter}, matcher::{fuzzy_match_completions, Matchable}, Completable, Completer};
...
where Self: Sized {
    ...
    fn completions(&self) -> Vec<impl super::Completable<'a, UnindexedBlockCompleter<'a, MarkdownLinkCompleter<'a>>>> where Self: Sized {
...
impl<'a> Completer<'a> for UnindexedBlockCompleter<'a, WikiLinkCompleter<'a>> {
    ...
    fn completions(&self) -> Vec<impl Completable<'a, Self>> where Self: Sized {
        let completables = self.completables(); let filter_text = self.grep_match_text();
...
impl<'a> UnindexedBlock<'a> {
    ...
    fn partial_completion<T: LinkCompleter<'a>>(&self, completer: &'a UnindexedBlockCompleter<'a, T>) -> Option<(String, CompletionItem)> {
        ...
        let (documentation, command, kind, label_detail, refname): (Option<Documentation>, Option<Command>, CompletionItemKind, Option<CompletionItemLabelDetails>, String) = match completer.link_completer.vault().select_referenceable_nodes(Some(block.file))
            .into_iter()
            .find(|referenceable| match referenceable {
                Referenceable::IndexedBlock(_path, indexed_block) => indexed_block.range.start.line == block.range.start.line,
                _ => false
            }) {
...
impl<'a> Completable<'a, UnindexedBlockCompleter<'a, MarkdownLinkCompleter<'a>>> for UnindexedBlock<'a> {
    fn completions(&self, completer: &UnindexedBlockCompleter<'a, MarkdownLinkCompleter<'a>>) -> impl Iterator<Item = CompletionItem> {
        (|| {
            ...
            Some(
                CompletionItem {
                    text_edit: Some(completer.link_completer.completion_text_edit(Some(&format!("${{1:{}}}", display)), &refname)),
                    filter_text: Some(completer.completion_filter_text(&completer.link_completer.entered_refname())),
...
impl<'a> Completable<'a, UnindexedBlockCompleter<'a, WikiLinkCompleter<'a>>> for UnindexedBlock<'a> {
    fn completions(&self, completer: &UnindexedBlockCompleter<'a, WikiLinkCompleter<'a>>) -> impl Iterator<Item = CompletionItem> {
        (|| {
            let (refname, partial_completion) = self.partial_completion(completer)?;
